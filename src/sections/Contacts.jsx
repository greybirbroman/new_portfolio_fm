...
const Contacts = () => {
  const ref = useRef();
  const isInView = useInView(ref, { margin: '-110px' });
  ...
  return (
    <m.div
      ...
      initial='initial'
      whileInView='animate'
      className='text-white flex flex-wrap items-center gap-10 pt-20 max-w-[1366px] mx-auto h-full'
    >
      <m.div className='flex-1' variants={variants}>
        <m.h4
          className='capitalize text-[100px] leading-[88px] font-bold '
          variants={variants}
        >
          {"let's work together"}
        </m.h4>
        <m.ul className='flex flex-col gap-5' variants={variants}>
          {contacts.map((item) => (
            <m.li key={item.id} variants={variants}>
              <p>{item.title}</p>
              <span>{item.span}</span>
            </m.li>
          ))}
      ...
      <div className='flex-1 relative'>
        ...
        >
          ...
          <button
            type='sumbit'
            className='py-5 px-5 bg-orange-500 color-black text-xl disabled:opacity-50'
